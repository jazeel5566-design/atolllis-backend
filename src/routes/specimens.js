...
router.post('/:specimenNumber/load-analyser', async (req, res) => {
  const specimen = await prisma.specimen.findUnique({
    where: { specimenNumber: req.params.specimenNumber },
...
router.post('/:specimenNumber/accept', async (req, res) => {
  const { acceptedBy } = req.body || {};
  if (!acceptedBy) return res.status(400).json({ error: 'acceptedBy is required' });
...
router.post('/:specimenNumber/reject', async (req, res) => {
  ...
  await prisma.specimen.update({ where: { id: specimen.id }, data: { status: 'rejected', rejectionReason: reason, acceptedAt: new Date() } });
