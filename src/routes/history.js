...
router.get('/', async (req, res) => {
  ...
  for (const p of patients) {
    const orders = await prisma.order.findMany({
      ...
      include: { tests: true, specimens: true },
    ...
    for (const o of orders) {
      ...
      const lines = tests.map(t => {
        ...
        if (t.status !== 'completed') return { code: t.code, name: t.name, status: t.status, specimenNumber };
        const tc = catalogTests.find(c => c.code === t.code);
        const info = tc ? evalResult(tc, p, t.value) : {};
        return { code: t.code, name: t.name, value: t.value, unit: t.unit, ...info, specimenNumber, validatedBy: t.validatedBy, validatedAt: t.validatedAt };
      });
      visibleOrders.push({ orderId: o.id, memoNumber: o.memoNumber, createdAt: o.createdAt, own, tests: lines });
    }
